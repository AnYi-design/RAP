...
>
> [@Semantics.timeZone: true](#semanticstimezone-true-and-semanticstimezonereference) - 指定为IANA 时区
>
> [@Semantics.timeZoneReference](#semanticstimezone-true-and-semanticstimezonereference) - 转换后的timestamp(必须是 UTC format)
...
## @Semantics.timeZone: true and @Semantics.timeZoneReference
> 涉及到时区转换:把当前时间转换成指定时区的时间，这个转换过程是在 RAP 后台发生的。
> 
> 根据目标时区字段 `SAPTimezone` 和当前时间字段 `Timestamp`，转换时间，转换后的时间显示在 `IANATimestamp` 字段，`IANATimezone` 时区是IANA 表现形式的时区。
> 
> `IANATimezone` 字段是IANA 形式时区; `IANATimestamp` 字段是转换后的时间。
> 
> 当 `SAPTimeZone` 字段发生变化，会改变 `IANATimeZone` 和 `IANATimestamp`（就是转化后时间）。
> 
> 当 `Timestamp` 字段发生变化，会根据 `SAPTimeZone` 改变 `IANATimestamp` 为正确的时间。
> 
cds
```
...
table
```
@EndUserText.label : 'YCXCREATEDATA'
@AbapCatalog.enhancement.category : #NOT_EXTENSIBLE
@AbapCatalog.tableCategory : #TRANSPARENT
@AbapCatalog.deliveryClass : #A
@AbapCatalog.dataMaintenance : #RESTRICTED
